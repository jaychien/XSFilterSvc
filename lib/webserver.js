...
var WebServer = function(options) {
    ...
    if (!_(options.xsserver).endsWith('/'))
        options.xsserver = options.xsserver + '/';
    ...
    self.xsservice = new XSService(options.xsserver);
    ...
    /*
        /api/getdata?symbol=2330.TW&freq=..&fields=..&count=..&fmt=..
        - freq = 8, 9, 10, ..
    ...
    app.get('/api/getdata', function(req, res) {
        ...
        self.xsservice.getHistData(symbolID, freq, fields, count, function(err, result) {
            if (err != null) {
    ...
    app.get('/api/getfield', function(req, res) {
        self.xsservice.getFieldData(function(err, fields) {
            if (err != null) {
                res.status(500).send(err);
            }
            else {
